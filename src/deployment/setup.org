...
#+BEGIN_SRC python :tangle scripts/setup.py :eval no
...
#+BEGIN_SRC python :tangle scripts/db_setup.py :eval no
...
#+BEGIN_SRC python :tangle scripts/migrate_old_to_new_data.py :eval
#!/usr/bin/python
import MySQLdb
import sys
...
#+BEGIN_SRC python :tangle scripts/pincode_migration.py :eval no 
import requests
import json
import yaml
...
#+BEGIN_SRC python :tangle scripts/migrate_phone.py :eval
#!/usr/bin/python                                                                                           
import MySQLdb
from src.app import create_app
from src.db import *
...
#+BEGIN_SRC python :tangle scripts/add_analytics.py :eval
#!/usr/bin/python                                                                                           
import MySQLdb
from src.app import create_app
from src.db import *
...
** Shell script to call the procedure.
   Following shell script will execute the
   =refresh_analytics_now= procedure which defined in the
   above section to update the analytics.
   #+BEGIN_SRC sh :tangle scripts/update_analytics.sh :eval no
   #!/bin/bash                                                                                                                                           
   ...
   #+END_SRC
** crontab
   The following commands will set the cronjob for updating
   analytics for every 4 hours.
   #+BEGIN_EXAMPLE
   cd /var/www/scripts
   chmod 755 update_analytics.sh
   crontab -l
   crontab -e
   * */4 * * * /var/www/scripts/update_analytics.sh
...
#+BEGIN_SRC sh :tangle scripts/setup.sh :eval no
#!/bin/bash
...
#+BEGIN_SRC sh :tangle scripts/configure.sh :eval no
#!/bin/bash
...
* Hardening apache security rules
   To protect server from the hackers following rules have
   been set on the apache server. Steps were taken from
   [[https://github.com/virtadpt/ubuntu-hardening/blob/master/14.04-lts/apache2/conf-available/security.conf][referrence]]
...
* Ubuntu 16.04 - Default MySQL 5.7 Configuration
  Unlike 14.04, 16.04 now splits up the configuration so it
is not all in the =/etc/mysql/my.cnf= file, but most of it is
in directories that are included as shown in the default
=my.cnf= file below:
** Default My.cnf file
#+BEGIN_SRC my.cnf
#
...
Most of the settings you are looking for are now found in
=/etc/mysql/mysql.conf.d/mysqld.cnf=. The contents is shown
below:
#+BEGIN_SRC mysqld.cnf
#
# The MySQL database server configuration file.
#
# You can copy this to one of:
...
#+END_SRC
2. Copy IAM user credentials who has read-write access to s3
   bucket.
#+BEGIN_SRC 
Access Key Id	Secret Access Key
<key id>	<secret key>
#+END_SRC
3. configure AWS credentials as follows 
#+BEGIN_SRC 
...
#+END_SRC
4. configure AWS config as follows
#+BEGIN_SRC 
~/.aws/config
#+END_SRC
#+BEGIN_SRC 
...
5. Run =aws configure= and enter above credentials. For
   region, use ='us-east-1¡¯=.

6. Check that you have access to s3 bucket with =aws s3 ls=.

7. Copy entire directory. Recommend using =sync= rather than
   =cp=. 
#+BEGIN_SRC 
