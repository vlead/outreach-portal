...
def populate_va_user_details_data():
    cursor1.execute("select * from va_user_details")
...
def populate_workshop_documents_data():
    cursor1.execute("select * from workshop_documents")
    data = cursor1.fetchall()

    workshop_documents_data = {}
    for row in data:
        workshop_documents_data['document_name'] = row[1]
        workshop_documents_data['document_path'] = row[2]
        workshop_documents_data['created_on'] = row[3]

        print workshop_documents_data

def populate_workshop_metirial_documents_data():
    # execute the SQL query using execute() method.
    cursor1.execute("select * from workshop_metirial_documents")
    # fetch all of the rows from the query
    data = cursor1.fetchall()

    workshop_metirial_documents_data = {}
    for row in data:
        workshop_metirial_documents_data['document_name'] = row[1]
        workshop_metirial_documents_data['document_path'] = row[2]
        workshop_metirial_documents_data['created_on'] = row[3]

        print workshop_metirial_documents_data

def populate_presentation_reporting_documents_data():
    # execute the SQL query using execute() method.
    cursor1.execute("select * from presentation_reporting_documents")
    # fetch all of the rows from the query
    data = cursor1.fetchall()

    presentation_reporting_documents_data = {}
    for row in data:
        presentation_reporting_documents_data['document_name'] = row[1]
        presentation_reporting_documents_data['document_path'] = row[2]
        presentation_reporting_documents_data['created_on'] = row[3]

        print presentation_reporting_documents_data

def populate_nodalcoordinatortraining_data():
    cursor1.execute("select * from nodalcoordinatortraining")
    data = cursor1.fetchall()

    nodalcoordinatortraining_data = {}
    for row in data:
        nodalcoordinatortraining_data['participants_attend'] = row[1]
        nodalcoordinatortraining_data['experiments_conducted'] = row[2]
        nodalcoordinatortraining_data['attendance_sheet'] = row[3]
        nodalcoordinatortraining_data['training_photos'] = row[4]
        nodalcoordinatortraining_data['outreach_id'] = row[7]
        nodalcoordinatortraining_data['created_on'] = row[8]

        print nodalcoordinatortraining_data

def populate_workshop_report_data():
    cursor1.execute("select * from workshop_report")
    data = cursor1.fetchall()

    workshop_report_data = {}
    for row in data:
        workshop_report_data['workshop_id'] = row[1]
        workshop_report_data['uid'] = row[2]
        workshop_report_data['participate_attend'] = row[3]
        workshop_report_data['participate_experiment'] = row[4]
        workshop_report_data['upload_attend_sheet'] = row[5]
        workshop_report_data['college_report'] = row[6]
        workshop_report_data['workshop_photos'] = row[7]
        workshop_report_data['comments_positive'] = row[8]
        workshop_report_data['comments_negative'] = row[9]
        workshop_report_data['created_on'] = row[12]

        print workshop_report_data
...
def populate_nodal_coordinator_details():
    ...
    for row in data:
        user1 = User.get_by_id(row[2])
        target_workshops = 0
        user2 = User.get_by_id(0)
        nodal_centre=NodalCentre.get_by_id(1)
        target_participants=row[1].strip()
        target_experiments=row[2].strip()
...
def populate_workshop_reports():
    ...
    cursor1.execute("select * from workshop_metirial_documents_data")
    data1 = cursor1.fetchall()

    cursor1.execute("select * from workshop_report_data")
...
def populate_reference_documents():
    ...
    for row in data1:
        ...
        for row in data2:
            ...
            User.query.filter_by(name=row[0]).first()
            new_reference_document = ReferenceDocuments(name=name, workshop=workshop,
                                                 path=path)
...
#    populate_workshop_data()
#    populate_workshop_metirial_documents_data()
#    populate_workshop_report_data()
#    populate_nodalcoordinatortraining_data()
#    populate_presentation_reporting_documents_data()
