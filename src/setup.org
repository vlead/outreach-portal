...
def populate_workshop_data():
    cursor1.execute("select * from workshop")
    data = cursor1.fetchall()

    workshop_data = {}
    for row in data:
         workshop_data['uid'] = row[12]
...
def populate_workshops():
    ...
    for row in data:
        ...
        user = User.get_by_id(row[12])
        participating_institutes = row[3].strip()
        no_of_participants_expected = row[5]
        no_of_sessions = row[6]
        labs_planned = row[9]
        ...
        print new_workshop.name, new_workshop.location, new_workshop.user.name, new_workshop.participating_institutes,
        print new_workshop.no_of_participants_expected, new_workshop.no_of_sessions,
       # new_workshop.save()
