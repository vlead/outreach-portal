...
UPLOAD_DIR_PATH = '/home/vlead/Desktop/outreach-docs/users/'
...
def save_file(id):
    ...
    if request.method == 'POST':
        file = request.files['file']
        filename = secure_filename(file.filename)
        dir_path = "%s" % ("/home/vlead/Desktop/outreach-docs/")
        file_path = "%s%s" % (dir_path, filename)
        file.save(file_path)

        path = {'path': file_path}
        print path
        print "http://localhost:5000/workshop_documents/"\
                                + id
        response = requests.put("http://localhost:5000/workshop_documents/"\
                                + id, data=path)
        print response
        return file_path
