...
#+BEGIN_SRC python :tangle src/db.py :eval no
    ...
    def __str__(self):
        return "Name = %s, e-mail id = %s,\
        institute_name = %s, role=%s, last_active = %s"\
        % (self.name, self.email, self.institute_name,\
           self.role.name, self.last_active)
...
def create_app(config):
    ...
    db.init_app(app)
    db.app = app
    os.environ["no_proxy"] = app.config['NO_PROXY']
...
def configure_errorhandlers(app):
    ...
    def bad_request(err):
        ...
        app.logger.error("error = %s, error code = 400" % str(err))
        return resp

    @app.errorhandler(404)
    def not_found(err):
        resp = make_response(jsonify(error=err.description), 404)
        app.logger.error("error = %s, error code = 404" % str(err))
        return resp

    @app.errorhandler(405)
    def method_not_allowed(err):
        resp = make_response(jsonify(error=err.description), 405)
        app.logger.error("error = %s, error code = 405" % str(err))
        return resp
    ...
    def server_error(err):
        ...
        app.logger.error("error = %s, error code = 500" % str(err))
        return resp
...
def configure_logging(app):
    ...
    os.system("chmod -R 777 %s" % log_file)
...
@api.route('/upload_reference_document/<id>', methods=['GET', 'POST'])
...
@api.route('/upload_workshop_report/<id>', methods=['GET', 'POST'])
