...
#+BEGIN_SRC python :tangle src/db.py :eval no
    ...
    def __str__(self):
        return "user = %s, last_updated = %s, target_workshops = %s, \
        target_participants = %s, target_experiments = %s, created_by = %s"\
            % (self.user.name, self.last_updated, self.target_workshops,\
               self.target_participants, self.target_experiments,\
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    ...
    def __str__(self):
        return "Name = %s, location = %s, created_by = %s, coordinators = %s" % \
            (self.name, self.location, self.created_by.name, self.coordinators.name)
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_nodal_centre_constructor(self):
        ...
        self.assertEqual(nc_details.user.name, nodal_centre.coordinators.name)
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_nodal_centres_get_all(self):
        ...
        self.assertEqual(user2, nodal_centres_list[0].created_by)
        self.assertEqual("IIIT-Hyd", nodal_centres_list[0].name)

        self.assertEqual(nc_details2, nodal_centres_list[1].coordinators)
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def to_client(self):
        return {
            'id': self.id,
            'created_by': self.user.to_client(),
            'coordinators': self.nodal_coordinator_detail.to_client(),
            'name': self.name,
            'location': self.location
        }
#+END_SRC
...
class TestNodalCentre(TestCase):
    ...
    def test_delete_nodal_centre(self):
        ...
        r = self.client.delete('/nodal_centres/1')
        r = self.client.get('/nodal_centres')
        result = json.loads(r.data)
