...
def check_authorization(email, key):
	...
	if session_list[email] == key:
	    current_app.logger.debug("Authorized user with email = %s key = %s" %
				     (email, key))
	    current_app.logger.debug("session_list = %s" %
				     (session_list))
	    return True
    except Exception, e:
	current_app.logger.error("session_list  %s" %
				 (session_list))
	current_app.logger.error("User not found in session_list  %s" %
				 (str(e)))
	return False
...
def login():
	...
	if verification_data['status'] == 'okay':
	    ...
	    current_app.logger.debug("session_list= %s" % session_list)
...
@api.route('/get_outreach_portal_usage', methods=['GET'])
def get_outreach_portal_usage():
    if request.method == 'GET':
        nc_url = APP_URL + "/nodal_centres"
        ws_url = APP_URL + "/workshops"
        ...
        try:
            if nc_response.ok and ws_response.ok and ws_usage_response.ok:
                ...
                for i in range(0, len(ws_usage_dict)):
                    if ws_usage_dict[i]['experiments_conducted'] != None :
                        usage += ws_usage_dict[i]['experiments_conducted']

                json_data = {"nodal_centres": len(nodal_centre_dict),
