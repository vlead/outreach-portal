...
***** Google OAuth Settings
      - All Google OAuth service required URLs should be specified
...
#+END_SRC
***** =Login= API
      - As soon as user invokes =login= button from f/e it will invokes
...
***** Google Callback response =API=
      - Once the Google verfied the user credentials it will send
        callback response to =/authorized= api which is registered at google's dashboard 
#+BEGIN_SRC python :tangle src/api.py :eval 
...
#+END_SRC
***** =root= API
      - This is the API which invoked as soon as user enters domain
