...
APP_URL = "http://outreach-demo.virtual-labs.ac.in/"
...
#+END_SRC
***** =Login= API
      - When clicks on =login= button from f/e it will invoke
...
#+END_SRC
***** =Logout= API
      - When clicks on =logout= button from f/e it will invoke
...
***** Google Callback response =API=
      - Once the Google verfied the user credentials it will send
        callback response to =/authorized= api which should be
        registered at google's dashboard as one of callback response
        api
      - Once the google verifies the =Authorization= then only
        =Outreach-Portal= will checks for =Authentication= and on successful it will be redirected to belonging user role api
...
def authorized():
    try:
        ...
        session['error'] = "Error in Google Authorization : "+str(e)            
        return redirect("/")
    try:
...
#+END_SRC
***** =Root= API
      - This API is invoked as soon as user enters domain
        name =http://outreach.vlabs.ac.in= or =http://outreach.virtual-labs.ac.in= and due to domain issue with Google Oauth service
        this api will always make sures that user will redirect to =http://outreach.virtual-labs.ac.in=
#+BEGIN_SRC python :tangle src/api.py :eval 
@api.route("/")
def index():
...
def modify_entity(entity, id):
    ...
    #if 'google_token' is not in session:
    #    abort(401, "Unauthorized Credentials")
