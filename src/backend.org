...
#+BEGIN_SRC python :tangle src/db.py :eval no 
    def get_user(self):
        return self.user
#+END_SRC
***** TestCreatedBy: WorkshopDocument --> User
...
#+BEGIN_SRC python :tangle src/db.py :eval no 
 #   @typecheck(path=str)
    def set_path(self, path):
        file = open(kwargs['path'], 'rb')
        file_name = secure_filename(file.name)
        print file_name
        file.write("/home/vlead/Desktop/uploads/")
        self.path = UPLOAD_DIR_PATH + file_name
#+END_SRC
...
UPLOAD_DIR_PATH = '/home/vlead/Desktop/uploads/'
...
*** Design
   ...
   2. The second URL pattern is to get details of a particular record,
      ...
      1. Get only few details of a particular workshop (where the workshop id
         is known before):
         #+BEGIN_EXAMPLE
	 GET /workshops/2?fields=name, conducted_by, participants_attended, status
	 #+END_EXAMPLE
