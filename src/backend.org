...
def admin():
    if request.method == 'POST':
        ...
        dir_path = "%s" % ("/home/dinnu93/Desktop/outreach-docs/")
        file_path = "%s%s" % (dir_path, filename)
        file.save(file_path)
        path = {'path': file_path}
        print path
        print "http://localhost:5000/workshop_documents/"+str(ID)
        response = requests.put("http://localhost:5000/workshop_documents/"+ str(ID), data=path)
        return redirect("/admin#documents")
    else:
        if ('email' in session) and (session['role'] == 'admin'):
            rg = requests.get("http://localhost:5000/workshop_documents?user_id="+str(session['id']))
...
class TestNodalCoordinatorDetail(TestCase):
    ...
    def test_update_existing_nc_details(self):
        print "test_update_existing_nc_details"
        role = Role(name=Name("OC"))
        role.save()
    ...
    def test_create_new_nc_details(self):
        ...
        r = self.client.post('/nodal_coordinator_details',
                             data=json.dumps({ 'user': user1.to_client(),
                                               'target_workshops':100,
                                               'target_participants':1000
                                           }))
        result = json.loads(r.data)
