...
#    college_reports = db.relationship('CollegeReport', backref='Workshop')
#+END_SRC
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    ...
    def __str__(self):
        return "last_updated = %s, user = %s, created_by = %s, target_workshops = %s, \
        target_participants = %s, target_experiments = %s" % \
            (self.last_updated, self.user, self.created_by,\
             self.target_workshops, self.target_participants,\
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_all_nc_details(self):
        ...
        nc_details1 = NodalCoordinatorDetail(user=user2, target_workshops=100,
                                             created_by=user1,
                                             target_participants=1000)
        nc_details1.save()
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_nc_details_by_id(self):
        ...
        self.assertEqual(nc_details1.get_by_id(1).user.name, "Will Smith")
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_nc_user(self):
        ...
        self.assertEqual(nc_user.name, "Will Smith")
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_nc_created_by(self):
        ...
        self.assertEqual(nc_created_by.name, "Joe Smith")
...
class NodalCentre(Entity):
    ...
    name = db.Column(db.String(128), nullable=False)
    location = db.Column(db.String(128))
    coordinators_id = db.Column(db.Integer, db.ForeignKey('nodal_coordinator_details.id'))
    created_by_id = db.Column(db.Integer, db.ForeignKey('users.id'))

    user = relationship('User', foreign_keys=[created_by_id])
    nc_detail = relationship('NodalCoordinatorDetail', foreign_keys=[coordinators_id])
...
#+BEGIN_EXAMPLE
    nodal_centre = NodalCentre(name=str,
                               location=str,                  
                               created_by=<instance of class User>,
                               coordinators=<instance of class NodalCoordinatorDetail>)
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def __init__(self, **kwargs):
        ...
        if 'coordinators' in kwargs:
            self.set_coordinators(kwargs['coordinators'])
        
        
    def __str__(self):
        return "Name = %s, location = %s, created_by = %s, coordinators = %s" % \
            (self.name, self.location, self.created_by, self.coordinators)
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_nodal_centre_constructor(self):
        ...
        nodal_centre = NodalCentre(name="IIIT-Hyd", location="Gachibowli",\
                                   created_by=user1, coordinators=nc_details)

        nodal_centre.save()
        self.assertEqual(user1.name, nodal_centre.created_by.name)
        self.assertEqual("IIIT-Hyd", nodal_centre.name)
        self.assertEqual("Gachibowli", nodal_centre.location)                        
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_nodal_centres_get_all(self):
        ...
        nodal_centre1 = NodalCentre(name="IIIT-Hyd", location="Gachibowli", \
                                    created_by=user_2, coordinators=nc_details1)
        nodal_centre1.save()

        nodal_centre2 = NodalCentre(name="HCU", location="Lingampalli", \
                                    created_by=user_2, coordinators=nc_details2)               
        nodal_centre2.save()
        
        nodal_centres_list = NodalCentre.get_all()

        print nodal_centres_list
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_get_nodal_centre_by_id(self):
        ...
        nodal_centre = NodalCentre(name="IIIT-Hyd", location="Gachibowli", \
                                   created_by=user, coordinators=nc_details)

        nodal_centre.save()

        self.assertEqual(nodal_centre.get_by_id(1).name, "IIIT-Hyd")
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_nodal_centre_created_by(self):
        ...
        centre_created_by = nodal_centre.get_created_by()
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_get_nodal_centre_name(self):
        ...
        centre_name = nodal_centre.get_name()
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_get_nodal_centre_location(self):
        ...
        centre_location = nodal_centre.get_location()
        self.assertEqual(centre_location, "Gachibowli")
#+END_SRC

***** getCoordinators: NodalCentre --> NodalCoordinatorDetails
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
...
***** Test getCoordinators: NodalCentre --> NodalCoordinatorDetails
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_get_nodal_centre_coordinators(self):
        print "test_get_nodal_centre_coordinators"
        role = Role(name=Name("OC"))
        role.save()
        user = User(name=Name("ambika"), email=Email("ambika@vlabs.ac.in"), role=role)
        user.save()
        ...
        centre_coordinators = nodal_centre.get_coordinators()
        self.assertEqual(centre_coordinators, nc_details)
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_set_nodalcentre_created_by(self):
        ...
        nodal_centre.set_created_by(user2)
        nodal_centre.save()

        centre = NodalCentre.get_by_id(1)
        self.assertEqual(centre.created_by, user2)
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_set_nodal_centre_name(self):
        ...
        nodal_centre.set_name("HCU")
        nodal_centre.save()
        centre = NodalCentre.get_by_id(1)
        self.assertEqual("HCU", centre.name)
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_set_nodal_centre_location(self):
        ...
        nodal_centre.save()
        nodal_centre.set_location("Hyderabad")
        nodal_centre.save()
        centre = NodalCentre.get_by_id(1)
        self.assertEqual("Hyderabad", centre.location)
#+END_SRC

***** setCoordinators: NodalCentre, NodalCoordinatorDetail --> Void
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    @typecheck(coordinators=NodalCoordinatorDetail)
    def set_coordinators(self, coordinators):
        self.coordinators = coordinators

#+END_SRC

***** Test setCoordinators: NodalCentre, NodalCoordinatorDetail --> Void
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def test_set_nodal_centre_coordinators(self):
        print "test_set_nodal_centre_coordinators"
        role1 = Role(name=Name("Admin"))
        role2 = Role(name=Name("OC"))
        user1 = User(name=Name("ambika"), email=Email("ambika@vlabs.ac.in"), role=role1)
        user2 = User(name=Name("madhavi"), email=Email("madhavi@vlabs.ac.in"), role=role2)
        ...
        nc_details1 = NodalCoordinatorDetail(user=user2, created_by=user1,\
                                            target_workshops=100,\
                                            target_participants=1000)

        nc_details1.save()

        nc_details2 = NodalCoordinatorDetail(user=user1, created_by=user1,\
                                            target_workshops=100,\
                                            target_participants=1000)
        ...
        nodal_centre = NodalCentre(name="IIIT-Hyd", location="Gachibowli", \
                                   created_by=user1, coordinators=nc_details1)

        nodal_centre.save()

        nodal_centre.set_coordinators(nc_details2)
        nodal_centre.save()
        centre = NodalCentre.get_by_id(1)
        self.assertEqual(centre.coordinators, nc_details2)
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no 
    def to_client(self):
        return {
            'id': self.id,
            'created_by': self.created_by.to_client(),
            'coordinators': self.coordinators.to_client(),
            'name': self.name,
            'location': self.location
        }
#+END_SRC
...
**** Constructor
#+BEGIN_EXAMPLE 
    college_report = CollegeReport(path=str,  workshop=<instance of class Workshop>)
...
#+BEGIN_SRC 
    def __init__(self, **kwargs):
        ...
        self.set_user(kwargs['workshop'])
        
        if 'path' in kwargs:
            self.set_path(kwargs['path'])

    def __str__(self):
        return "workshop = %s, path = %s" % \
            (self.workshop.name, self.path)
...
**** Tests for College Report Constructor
#+BEGIN_SRC 
class TestCollegeReport(TestCase):
    TESTING = True
...
#+END_SRC
#+BEGIN_SRC 
    def test_college_report_constructor(self):
        print "test_college_report_constructor"
        ...
        college_report = CollegeReport(workshop=workshop)
        college_report.save()
         
        # Calling the college_report constructor without any arguments should
        # raise AttributeRequired exception
        self.assertRaises(AttributeRequired, CollegeReport)

        # Calling the college_report constructor with necessary arguments
        # (workshop,path) but with wrong type should raise TypeError
        self.assertRaises(TypeError, CollegeReport,
                          ,**{'path': 12
                             'workshop': workshop
                            })

#+END_SRC
**** Functions that operate on the College Report domain
***** getAll: Nil --> set[CollegeReport]
#+BEGIN_SRC 
    @staticmethod
    def get_all():
        return CollegeReport.query.all()

#+END_SRC

***** Test getAll: Nil --> set[CollegeReport]
#+BEGIN_SRC 
    def test_college_reports_get_all(self):
        print "test_college_reports_get_all"
        ...
        doc = CollegeReport(path="/static/uploads/college-reports/file.pdf" workshop=workshop)
        doc.save()

        self.assertEqual("/static/uploads/college-reports/file.pdf", doc.path)
#+END_SRC

***** getCollegeReport: Id --> CollegeReport
#+BEGIN_SRC 
    @staticmethod
    def get_by_id(id):
        return CollegeReport.query.get(id)

#+END_SRC

***** Test getCollegeReport: Id --> CollegeReport
#+BEGIN_SRC 
    def test_get_college_report_by_id(self):
        print "test_get_document_by_id"
        ...
        college_report = CollegeReport(path="/static/uploads/college-reports/file.pdf" workshop=workshop)
        college_report.save()

        self.assertEqual(college_report.get_by_id(1).path, "/static/uploads/college-reports/file.pdf" )

#+END_SRC
     
***** getPath: CollegeReport --> String
#+BEGIN_SRC 
    def get_path(self):
        return self.path
#+END_SRC

***** Test getPath: CollegeReport --> String
#+BEGIN_SRC 
    def test_get_college_report_path(self):
        print "test_get_college_report_path"
        name = Name("Admin")
        ...
        college_report_path = college_report.get_path()
        self.assertEqual(college_report_path, "/static/uploads/college-reports/file.pdf")
#+END_SRC
     
***** getWorkshop: CollegeReport --> Workshop
#+BEGIN_SRC 
    def get_workshop(self):
        return self.workshop
#+END_SRC
***** TestWorkshop: CollegeReport --> Workshop
#+BEGIN_SRC 
def test_get_workshop(self):
        ...
        workshop = workshop_1.get_workshop()
        self.assertEqual("Coordinators Training", workshop.name )

#+END_SRC
***** setPath: CollegeReport, String --> Void
#+BEGIN_SRC 
    @typecheck(path=str)
    def set_path(self, path):
        self.path = path
...
***** Test setPath: CollegeReport, String --> Void
#+BEGIN_SRC 
    def test_set_college_report_path(self):
        print "test_set_college_report_path"
        ...
        college_report.set_path("/static/uploads/college-reports/file.doc")
        college_report.save()

        doc = college_report.get_by_id(1)
        self.assertEqual(doc.path, "/static/uploads/college-reports/file.doc")
#+END_SRC
      
***** setWorkshop: CollegeReport, Workshop --> Void
#+BEGIN_SRC 
    @typecheck(user=User)
...
#+END_SRC
***** Test setWorkshop: CollegeReport, Workshop --> Void
#+BEGIN_SRC 
    def test_set_workshop(self):
        print "test_set_workshop"
        role = Role(name=Name("Admin"))
        ...
        college_report = CollegeReport(path="/static/uploads/college-reports/file.pdf" workshop=workshop1)
        college_report.save()
     
        college_report.set_workshop(workshop2)
        college_report.save()
        self.assertEqual(college_report.workshop.location, "IIT-D")
#+END_SRC
***** to_client
#+BEGIN_SRC 
    def to_client(self):
        return {
            'id': self.id,
            'path': self.path,
            'wokshop': self.workshop.to_client()
        }
#+END_SRC
      
***** Test CollegeReport to_client
#+BEGIN_SRC 
    def test_college_report_to_client(self):
        print "test_college_report_to_client"
...
#college_report = {'entity_class': CollegeReport,
#                     'attributes': [
 #                        {'name': 'id',
  #                        'class': None,
   #                       'relationship': 'one'},
...
#college_reports = college_report
...
def admin():
    if request.method == 'POST':
        ...
        r = requests.post(APP_URL+"/workshop_documents",data=name_data)
        ...
        path = {'path': file_path}
        print path
        response = requests.put(APP_URL + "/workshop_documents/"+ str(ID), data=path)
        return redirect("/admin#documents")
    else:
        if ('email' in session) and (session['role'] == 'admin'):
            rg = requests.get(APP_URL + "/workshop_documents?user_id="+str(session['id']))
...
entity_map_types = {
    ...
    'nodal_centres': {
        ...
        'types': {
            'created_by': User,
            'coordinators': NodalCoordinatorDetail,
            'name': str,
            'location': str
        }
    }
...
class TestNodalCentre(TestCase):
    ...
    def test_get_all_nodal_centres(self):
        ...
        nc_details1 = NodalCoordinatorDetail(user=user1, target_workshops=100,\
                                             target_participants=1000)
        nc_details1.save()
        ...
        nodal_centre1 = NodalCentre(name="IIIT-Hyd", location="Gachibowli", \
                                    created_by=user2, coordinators=nc_details1)

        nodal_centre1.save()   
 
        nodal_centre2 = NodalCentre(name="HCU", location="Gachibowli", \
                                    created_by=user2, coordinators=nc_details2)               
    ...
    def test_create_new_nodal_centre(self):
        ...
        r = self.client.post('/nodal_centres',
                             data=json.dumps({ 'created_by': user1.to_client(),
                                               'coordinators': nc_details1.to_client(),
                                               'name': 'IIIT-Hyd',
                                               'location': 'Gachibowli'
                                           }))
        result = json.loads(r.data)
