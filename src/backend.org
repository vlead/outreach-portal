...
#        if not isinstance(kwargs['created'], Datetime):
#            raise TypeError("created should be of type Datetime")
#        self.created = kwargs['created']
    ...
    def __str__(self):
        return "Name = %s, e-mail id = %s, role = %s, last_active = %s, created = %s" % \
            (self.name, self.email, self.role.name, self.last_active, self.created)
...
***** setLastActive: User, Datetime --> Void
#+BEGIN_SRC python :tangle src/db.py :eval no
#    @typecheck(email=str)
