...
def get_college_name(mac_addr):
    ...
    current_app.logger.error("ELASTIC_URL %s" % (ELASTIC_URL))
    ...
    if colleges.status_code == 200:
        for college in colleges.json()['hits']['hits']:            
            ...
            current_app.logger.error("college_details %s" % (mac_addr_of_elastic_data))
            if mac_addr == mac_addr_of_elastic_data:
                return str(college['_source']['college_details']['college_name'])
...
def get_usage():
    ...
    if request.method == 'POST':        
        ...
        if usages.status_code == 200:
            current_app.logger.error("usages %s" % (usages.json()))
            for feedback in usages.json()['hits']['hits']:
                date_of_elastic_data = feedback['_source']['date']
                if version == "offline":
                    current_app.logger.error("date %s" % (feedback['_source']['date']))
                    if date == date_of_elastic_data:
                        current_app.logger.error("true")
                        count = count+1                        
                    
                else:
                    gateway_ip_of_e_db = feedback['_source']['gateway_ip']
                    current_app.logger.error("gateway_ip %s" % (feedback['_source']['gateway_ip']))
...
"""
@api.route("/ws_details/<id>")
def detailed_ws_details(id):
    r = requests.get("%s/workshop_reports?workshop_id=%s" % (APP_URL, id))
    ws_reports = r.json()
    return render_template("detailed_ws_details.html", data=ws_reports)

@api.route("/workshop_details")
def workshop_details():
    return_dict = {}
    if request.method == 'GET':
	approved_workshops_url = APP_URL + "/workshops?status_id=3"
	approved_workshops = requests.get(usage_url)
	try:
	    if approved_workshops.ok:
		ws = approved_workshops.text
		ws_dict = json.loads(ws)
		ws_report_details = []
		for workshop in ws_dict:
		   # print"here"
		    workshop_id = workshop['id']
		    workshop_reports_url = APP_URL + "/workshop_reports?workshop_id=" + str(workshop_id)
		    user_response = requests.get(workshop_reports_url)
		    try:
			if user_response.ok:
			    users = user_response.text
			    users_dict = json.loads(users)
			    workshop["reports"] = users_dict
		    except Exception as e:
			print str(e)
	except Exception as e:
	    print str(e)
