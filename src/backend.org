  ...
  def upload_file(file, url):
      if file and allowed_file(file.filename):
          ...
          if response.status_code != 200:
              ...
              current_app.logger.error("Unable to save file path in database")
              response = requests.delete(url) # Delete the record in database
              try:
                  ##delete the file
                  delete_file(file_path)
              except Exception, e:
                  current_app.logger.error("Unable to delete the file from the filesystem")
                  abort(500, 'error is %s' % (str(e)))
