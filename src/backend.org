...
class Workshop(Entity):
    ...
    conducted_on = db.Column(db.String(60))
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def __init__(self, **kwargs):
        ...
        if 'conducted_on' not in kwargs:
            raise AttributeRequired("mandatory attribute `conducted_on` is\
            missing")
        self.set_conducted_on(kwargs['conducted_on'])
...
#+END_SRC
***** getConductedOn: Workshop --> Conducted_on
#+BEGIN_SRC python :tangle src/db.py :eval no
    def get_conducted_on(self):
        current_app.logger.debug("get conducted_on: %s" % self.conducted_on)
        return self.conducted_on
#+END_SRC
...
#+END_SRC
***** setConductedOn: Workshop, Conducted_on --> Void
#+BEGIN_SRC python :tangle src/db.py :eval no
    @typecheck(conducted_on=str)
    def set_conducted_on(self, conducted_on):
        self.conducted_on = conducted_on
