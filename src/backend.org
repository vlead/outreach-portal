...
from maps import *
from outreach_logging import OutreachLogger

outreach_logger = OutreachLogger()
app_logger = outreach_logger.get_logger()
...
def configure_errorhandlers(app):
    ...
    def server_error(err):
        app_logger.error("error code = %s" % "500")
        resp = None
        try:
            app_logger.error("error desc = %s" % err.description)
            resp = make_response(jsonify(error=err.description), 500)
        except Exception:
            try:
                app_logger.error("error mesg = %s" % err.message)
                resp = make_response(jsonify(error=err.message), 500)
            except Exception:
                resp = make_response(jsonify(error=str(err)), 500)
                app_logger.error("error = %s" % str(err))
        return resp

    @app.errorhandler(405)
    def method_not_allowed(err):
        app_logger.error("error code = %s" % "405")
        resp = None
        try:
            app_logger.error("error desc = %s" % err.description)
    ...
    def not_found(err):
        app_logger.error("error code = %s" % "404")
        resp = None
        try:
            app_logger.error("error desc = %s" % err.description)
    ...
    def bad_request(err):
        app_logger.error("error code = %s" % "400")
        resp = None
        try:
            app_logger.error("error desc = %s" % err.description)
...
** Logging 
#+BEGIN_SRC python :tangle src/outreach_logging.py :eval no 
import logging
...
class OutreachLogger:
    ...
    outreach_logger = None

    def __init__(self):

        self.outreach_logger = logging.getLogger("outreach")
        ...
        os.system("chmod 777 %s" % log_file)
        max_size = 1024 * 1024 * 20  # Max Size for a log file: 20MB
        log_handler = logging.handlers.RotatingFileHandler(log_file,
                                                            maxBytes=max_size,
                                                            backupCount=10)
        log_handler.setFormatter(formatter)
        self.outreach_logger.addHandler(log_handler)
        self.outreach_logger.setLevel(LOG_LEVEL)

    def get_logger(self):
        return self.outreach_logger
...
def login():
    if request.method == 'POST':
        ...
        app_logger.debug("response from Persona = %s" %
                                 resp.json())
        if resp.ok:
        ...
        if verification_data['status'] == 'okay':
            ...
            app_logger.debug("url_for_getting_the_user = %s" %
                                     url_for_getting_the_user)
            backend_resp = requests.get(url_for_getting_the_user)
            app_logger.debug("backend response = %s" %
                                     backend_resp.text)
